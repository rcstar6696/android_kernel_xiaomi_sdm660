		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if ((chg->typec_mode == POWER_SUPPLY_TYPEC_SOURCE_DEFAULT ||
			...
			val->intval = 1;
		if (strstr(saved_command_line,"androidboot.mode=charger") == NULL && (chg->real_charger_type == POWER_SUPPLY_TYPE_UNKNOWN))
			val->intval = 0;
		break;
...
#if defined(CONFIG_KERNEL_CUSTOM_E7T)
	if(strstr(saved_command_line,"androidboot.mode=charger") == NULL && (stat != 0x01)) {
